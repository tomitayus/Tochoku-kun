...
| **C-H列** | 2-7 | 大学休日 | 土日祝の大学系当直 | **カテ当番必須** |
...
| **1** | 平日大学緩和フラグ | B/I-K列でカテなしでも配置可 |
| **3** | 比率除外フラグ | BG/HT比率計算から除外 |
| **0 / 空白** | カテなし | C-H自由配置可、カテ制約なし |

**コード体系の整理:**
- **アルファベット（A〜E, CC）**: カテ当番を示す。その日は大学系優先
- **数字（1, 3）**: 特殊フラグ。配置ロジックへの影響を制御
- **0 / 空白**: カテなし。制約が最も緩い
...
| ABS-005 | 同日重複禁止 | 同一医師が同一日に2枠以上 | ロジックで排除 |
| ABS-006 | 水曜日L〜Y禁止医師 | 指定医師（金城・山田・野寺）が水曜日にL〜Y列配置 | `WED_FORBIDDEN_DOCTORS`で除外 |
...
| ID | 制約名 | 違反判定 | 処理 |
|----|--------|----------|------|
| HARD-001 | TARGET_CAP超過 | `assigned_count > TARGET_CAP[doc]` | パターン除外 |
| HARD-002 | gap違反 | 連続当直の間隔 < 3日 | パターン除外 |
| HARD-003 | 未割当枠 | 空き枠が残存 | パターン除外 |
| HARD-004 | CODE_2のn+1違反 | CODE_2医師が`BASE_TARGET`超過 | パターン除外 |

> **注**: ハード制約は違反数=0が必須。1件でも違反があればパターン全体を却下する。
...
| ID | 制約名 | 緩和フラグ | 緩和順序 |
|----|--------|-----------|---------|
| SEMI-001 | 平日大学系カテ要件 | `relax_bh_limit=True` | 2番目 |
| SEMI-002 | 休日大学系カテ当番 | `relax_ch_kate=True` | 3番目 |
| SEMI-003 | gap制約 | `relax_gap=True` | 4番目 |
| SEMI-004 | 大学最低1回 | - | 修正フェーズで対応 |

**緩和フラグの命名について（歴史的経緯）:**
> - `relax_bh_limit`: 本来は「B-K列カテ制限」を指すが、初期実装時の命名が残存。実際には**平日大学系（B列+I-K列）全体**のカテ要件緩和を制御
> - `relax_ch_kate`: C-H列のカテ当番要件の緩和を制御（命名は正確）

**緩和適用フロー:**
```
1. 通常制約でチェック
   ↓ 候補なし
2. relax_bh_limit=True で再試行（SEMI-001緩和）
   ↓ 候補なし
3. relax_ch_kate=True で再試行（SEMI-002緩和）
   ↓ 候補なし
4. relax_gap=True で再試行（SEMI-003緩和）
```

**SEMI-004「大学最低1回」の詳細:**
```
対象: CODE_1.2医師（sheet2にコード1.2を持つ医師）
     └─ 全active医師ではなく、CODE_1.2医師のみが対象
処理:
  1. パターン生成時: 考慮しない（配置優先度で調整）
  2. 修正フェーズ: fix_code_1_2_violations() で代替配置を試行
  3. 代替不可の場合: SOFT-004としてペナルティ150を加算
```

### 2.4 ソフト制約（ペナルティ加算）

#### 2.4.1 ほぼ禁忌（W≥150: 事実上のハード制約）

高ペナルティにより、実質的に配置を回避する制約:

| ID | 制約名 | ペナルティ重み | 説明 |
|----|--------|---------------|------|
| SOFT-001 | 外病院0回 | 300 | active医師は外病院最低1回必須 |
| SOFT-002 | 大学3回以上 | 150 | `bg_count >= 3`（CC除く） |
| SOFT-003 | 外病院同一病院重複 | W_EXTERNAL_HOSP_DUP=150 | 同一外病院に2回以上 |
| SOFT-004 | CODE_1.2大学0回 | 150 | 大学最低1回未達（SEMI-004緩和後） |

#### 2.4.2 中程度の回避（W=51-100）

| ID | 制約名 | ペナルティ重み | 説明 |
|----|--------|---------------|------|
| SOFT-005 | gap違反 | W_GAP=100 | 修正後に残った間隔違反 |
| SOFT-006 | BG/HT差3以上 | 100 | 大学と外病院の不均衡 |
| SOFT-007 | 大学平日2回以上 | 80 | 平日偏り回避 |

#### 2.4.3 軽微な非推奨（W=1-50）

| ID | 制約名 | ペナルティ重み | 説明 |
|----|--------|---------------|------|
| SOFT-008 | 公平性 | W_FAIR_TOTAL=30 | `max(割当数) - min(割当数)` の差 |
| SOFT-009 | 大学ばらつき | W_BG_SPREAD=3 | 累計の標準偏差 |
| SOFT-010 | 外病院ばらつき | W_HT_SPREAD=3 | 累計の標準偏差 |
| SOFT-011 | 休日ばらつき | W_WE_SPREAD=3 | 累計の標準偏差 |
| SOFT-012 | 平日ばらつき | W_WD_SPREAD=2 | 累計の標準偏差 |
| SOFT-013 | B-K/L-Y比率バランス | W_BK_LY_BALANCE=2 | 1:1に近づける |

#### 2.4.4 許容（W=0）

| ID | 制約名 | ペナルティ重み | 説明 |
|----|--------|---------------|------|
| SOFT-014 | 大学同一病院重複 | W_HOSP_DUP=0 | 許容（ペナルティなし） |
...
### 4.2 休日大学系（C-H列）の配置ルール

**配置可能条件（OR条件）:**
1. その日にカテ当番がある（sheet3でアルファベットあり）
2. カテなし医師（sheet3に1つもアルファベットがない）

> **関連制約**: SEMI-002（緩和時は上記条件を無視）

### 4.3 平日大学系（B列 + I-K列）の配置ルール

**配置可能条件（OR条件）:**
1. カテなし医師
2. その日にカテ当番がある
3. sheet3で「1」を持つ医師（SEMI-001緩和対象）

> **関連制約**: SEMI-001（緩和時は上記条件を無視）
> **列の内訳**: B列（インデックス1）、I列（8）、J列（9）、K列（10）
...
| **Phase 3: 必須配置** | #4-#6 | 大学最低1回・カテ制約を満たす |
| **Phase 4: バランス** | #7-#11 | BG/HT, gap, 重複を順次調整 |
| **Phase 5: 微調整** | #12-#13 | 公平性調整と残枠埋め（最後） |

**ループ実行:** なし（1回のみ実行）。各修正関数は収束まで内部でループする。
...
| カテゴリ | ID範囲 | 説明 |
|---------|--------|------|
| **ABS** | ABS-001〜006 | 絶対禁忌（配置不可） |
| **HARD** | HARD-001〜004 | ハード制約（パターン除外） |
| **SEMI** | SEMI-001〜004 | 準ハード制約（緩和可） |
| **SOFT** | SOFT-001〜014 | ソフト制約（ペナルティ） |

### 列インデックス対応表

| 列名 | Index | 分類 | カテ制約 | 関連制約ID |
|------|-------|------|---------|-----------|
| B | 1 | 平日大学系 | 緩和可 | SEMI-001 |
| C-H | 2-7 | 休日大学系 | カテ当番必須 | SEMI-002 |
...
| CODE_2 | sheet2にコード2あり | ABS-002, HARD-004 |
| CODE_3 | sheet2にコード3あり | ABS-003 |
| CODE_1.2 | sheet2にコード1.2あり | SEMI-004, SOFT-004 |
| カテ保有 | sheet3にアルファベットあり | ABS-004 |
| カテなし | sheet3にアルファベットなし | - |
| SHEET3「1」 | sheet3に「1」あり | SEMI-001緩和 |
| SHEET3「3」 | sheet3に「3」あり | 比率計算除外 |
