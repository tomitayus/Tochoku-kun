...
| 6 | 水曜日L〜Y禁止医師 | 指定医師（金城・山田・野寺）が水曜日にL〜Y列配置 | `WED_FORBIDDEN_DOCTORS`で除外 |
...
| # | 制約名 | 違反判定 | 処理 |
|---|--------|----------|------|
| 1 | TARGET_CAP超過 | `assigned_count > TARGET_CAP[doc]` | パターン除外 |
| 2 | gap違反 | 連続当直の間隔 < 3日 | パターン除外 |
| 3 | 未割当枠 | 空き枠が残存 | パターン除外 |
| 4 | CODE_2のn+1違反 | CODE_2医師が`BASE_TARGET`超過 | パターン除外 |
...
| # | 制約名 | ペナルティ重み | 説明 |
|---|--------|---------------|------|
| 1 | 外病院0回 | 300 | active医師は外病院最低1回必須 |
| 2 | 大学3回以上 | 150 | `bg_count >= 3`（CC除く） |
| 3 | 外病院同一病院重複 | W_EXTERNAL_HOSP_DUP=150 | 同一外病院に2回以上 |
| 4 | CODE_1.2大学0回 | 150 | 大学最低1回未達（準ハード緩和後） |

#### 2.4.2 中程度の回避（W=51-100）

| # | 制約名 | ペナルティ重み | 説明 |
|---|--------|---------------|------|
| 5 | gap違反 | W_GAP=100 | 修正後に残った間隔違反 |
| 6 | BG/HT差3以上 | 100 | 大学と外病院の不均衡 |
| 7 | 大学平日2回以上 | 80 | 平日偏り回避 |

#### 2.4.3 軽微な非推奨（W=1-50）

| # | 制約名 | ペナルティ重み | 説明 |
|---|--------|---------------|------|
| 8 | 公平性 | W_FAIR_TOTAL=30 | `max(割当数) - min(割当数)` の差 |
| 9 | 大学ばらつき | W_BG_SPREAD=3 | 累計の標準偏差 |
| 10 | 外病院ばらつき | W_HT_SPREAD=3 | 累計の標準偏差 |
| 11 | 休日ばらつき | W_WE_SPREAD=3 | 累計の標準偏差 |
| 12 | 平日ばらつき | W_WD_SPREAD=2 | 累計の標準偏差 |
| 13 | B-K/L-Y比率バランス | W_BK_LY_BALANCE=2 | 1:1に近づける |

#### 2.4.4 許容（W=0）

| # | 制約名 | ペナルティ重み | 説明 |
|---|--------|---------------|------|
| 14 | 大学同一病院重複 | W_HOSP_DUP=0 | 許容（ペナルティなし） |
